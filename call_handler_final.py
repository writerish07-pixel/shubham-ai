"""
call_handler_final.py — Call session manager with self-learning integration (FIXED).

🔥 FIX: This is the corrected version of call_handler_learning.py.
ISSUES FIXED:
1. Imported `voice_optimized` — RENAMED to `voice` in PR #13
2. Imported `intent_optimized` — RENAMED to `intent` in PR #13
3. Imported `config_learning` — which itself imported `config_optimized` (deleted)
   Now imports `config_final` which correctly imports from `config`
4. Used `is_inbound=(direction != "outbound")` instead of the pre-computed variable

SELF-LEARNING FEATURES (from call_handler_learning.py):
...
To activate: Replace call_handler.py with this file (or update main imports).
"""
import asyncio
import logging
import re
...
# 🔥 FIX: Import from `voice` (was `voice_optimized` — renamed in PR #13)
from voice import (
    transcribe_audio, synthesize_speech,
    synthesize_speech_async, transcribe_audio_async,
)
# 🔥 FIX: Import from `config_final` (was `config_learning` which imported deleted `config_optimized`)
import config_final as config
import sheets_manager as db

log = logging.getLogger("shubham-ai.call-handler-final")
...
# ══════════════════════════════════════════════════════════════════════════════
# 🔥 SELF-LEARNING: Session management with learning-enabled conversation
# ══════════════════════════════════════════════════════════════════════════════
...
def start_call_session(call_sid: str, caller_number: str,
    ...
    🔥 FIX: Uses ConversationManagerLearning instead of ConversationManager
    — enables RAG context injection and competitor detection.
    🔥 FIX: Uses pre-computed is_inbound variable consistently.
    """
    lead = None
    ...
    # 🔥 FIX: Compute is_inbound once, use it everywhere (was duplicated/inconsistent)
    is_inbound = (
        direction != "outbound"
        if direction
    ...
    # 🔥 FIX: Use learning-enabled conversation manager with correct is_inbound
    session = {
        "call_sid": call_sid,
...
# ══════════════════════════════════════════════════════════════════════════════
# 🔥 SELF-LEARNING: Speech processing with learning-enabled agent
# ══════════════════════════════════════════════════════════════════════════════
...
async def process_customer_speech_async(call_sid: str, audio_bytes: bytes) -> bytes:
    ...
    🔥 SELF-LEARNING: The ConversationManagerLearning.chat() method
    ...
    # 🔥 FIX: Import from `intent` (was `intent_optimized` — renamed in PR #13)
...
def process_customer_speech(call_sid: str, audio_bytes: bytes) -> bytes:
    ...
    # 🔥 FIX: Import from `intent` (was `intent_optimized`)
    from intent import detect_intent
    intent_response = detect_intent(customer_text, lead=session.get("lead"))
...
# ══════════════════════════════════════════════════════════════════════════════
# 🔥 SELF-LEARNING: End call with async learning pipeline
# ══════════════════════════════════════════════════════════════════════════════
...
def end_call_session(call_sid: str, duration_sec: int = 0) -> dict:
    ...
    🔥 SELF-LEARNING: After analyzing the call and updating the lead,
...
def _fire_learning_pipeline(transcript: str, call_sid: str,
    ...
    """
    🔥 SELF-LEARNING: Fire the learning pipeline as a background task.
