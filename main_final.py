"""
main_final.py — Shubham Motors AI Voice Agent (FINAL — LEARNING INTEGRATED)
FastAPI server handling all Exotel webhooks, admin dashboard, lead import, offer upload.

🔥 FIX: This is the corrected version of main.py with learning system FULLY INTEGRATED.
ISSUES FIXED:
1. Imported from `call_handler` (base) → now imports from `call_handler_final` (learning-enabled)
2. Imported from `agent` (base) → now imports from `agent_learning` (RAG-enhanced)
3. Used `config` (base) → now uses `config_final` (includes learning constants)
4. `call_status()` endpoint did NOT trigger learning pipeline → now fires async learning
5. No document learning API → added `/api/documents/upload` endpoint
6. No learning status in health check → added learning system status

To activate: Replace main.py with this file (or rename).
...
- Learning pipeline runs ASYNC in background — zero impact on call latency.
"""
import base64
...
# Module imports
# 🔥 FIX: Use config_final (includes learning constants like LEARNING_ENABLED, VECTOR_DB_DIR, etc.)
import config_final as config
import sheets_manager as db
# 🔥 FIX: Use call_handler_final (learning-enabled ConversationManagerLearning + async learning pipeline)
from call_handler_final import (
    start_call_session, get_opening_audio,
    end_call_session, active_calls
)
# 🔥 FIX: Use agent_learning (RAG-enhanced opening messages + sales psychology prompts)
from agent_learning import get_opening_message
...
async def lifespan(app: FastAPI):
    keep_alive()
    print(f"\n{'='*60}")
    print("  SHUBHAM MOTORS AI AGENT — FINAL BUILD (LEARNING ACTIVE)")
    ...
    # 🔥 FIX: Show learning system status at startup
...
# ── App setup ──────────────────────────────────────────────────────────────────
# 🔥 FIX: Updated title and version to reflect learning integration
app = FastAPI(title="Shubham Motors AI Agent (Final + Learning)", version="4.0.0", lifespan=lifespan)
...
async def root():
    return JSONResponse({
        "status": "running",
        "agent": "Shubham Motors AI Voice Agent (Final + Learning)",
        "version": "4.0.0",
        "dashboard": f"{config.PUBLIC_URL}/dashboard",
        # 🔥 FIX: Include learning system status in health response
...
@app.get("/health")
async def health():
    # 🔥 FIX: Include learning system status in health check
    return {
        "status": "ok",
        "learning_enabled": config.LEARNING_ENABLED,
...
# ══════════════════════════════════════════════════════════════════════════════
# 🔥 FIX: Document learning endpoint — connects document_learning.py to main flow
# Previously, document_learning.py existed but had NO API endpoint to trigger it.
# ══════════════════════════════════════════════════════════════════════════════
...
async def upload_document(
    ...
    """
    🔥 FIX: New endpoint to upload documents (PDF/JPEG/Excel) for learning.
    ...
    try:
        from document_learning import process_document
...
# 🔥 FIX: Learning system status endpoint — lets admin check if learning is working
...
async def learning_status():
    ...
    try:
        ...
        stats = get_stats()
        status["vector_db_entries"] = stats.get("total_entries", 0)
...
# 🔥 FIX: Updated entrypoint to use main_final module
if __name__ == "__main__":
    uvicorn.run("main_final:app", host="0.0.0.0", port=config.PORT, log_level="info")
